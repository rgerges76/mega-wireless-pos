...
function sendToPOS(id){var repairs=ensureRepairs(),r=repairs.find(function(x){return x.id===id});if(!r)return;var balance=Math.max(0,Number(r.total||0)-Number(r.deposit||0));if(balance<=0){alert('This repair has no balance due.');return}provisionOldPOSRepair(r,balance);provisionNewPOSRepair(r,balance);r.status='Ready for Pickup';r.updatedAt=new Date().toISOString();saveRepairs(repairs);location.href='index.html'}
